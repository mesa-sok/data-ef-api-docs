...
def main(dataset_id: str, top_k: int, json_output: bool) -> None:
    ...
    if dataset_id not in records:
        raise SystemExit(f"dataset_id '{dataset_id}' not found in metadata")

    client = QdrantClient(path=str(QDRANT_PATH))
    target, matches = get_nearest_neighbors(client, records, dataset_id, top_k)
